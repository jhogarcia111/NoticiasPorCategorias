...
#### **Características:**
- ✅ Cierra automáticamente procesos en los puertos específicos del proyecto (3017 y 4017)
...
2. **Ejecutar el archivo .bat:**
   ```bash
   restart-server-Noticias_por_Categor_as__Linkedin.bat
   ```
...
**⚠️ CRÍTICO:** Solo procesar features del proyecto 17 (Noticias por Categorías- Linkedin) que estén en estado `pendiente` o `en_correccion`. NO procesar features de otros proyectos ni features archivadas/aprobadas.

```javascript
// Obtener features pendientes SOLO del proyecto 17 (Noticias por Categorías- Linkedin)
const response = await fetch('http://localhost:3005/api/project-tracker/features?projectId=17');
const features = await response.json();

// Filtrar SOLO features pendientes o en corrección del proyecto 17
const pendingFeatures = features.filter(f => 
  f.projectId === 17 && // ⚠️ SOLO proyecto 17 (Noticias por Categorías- Linkedin)
...
3. **Verificar antes de procesar**: Siempre verificar que `feature.projectId === 17` y que el estado es `pendiente` o `en_correccion`
...
- [ ] ⚠️ **VERIFICAR que `feature.projectId === 17`** (Noticias por Categorías- Linkedin únicamente)
...
curl -X POST http://localhost:3005/api/project-tracker/features \
  ...
  -d "{\"projectId\": 17, \"featureName\": \"Nueva Funcionalidad\", \"description\": \"Implementación de nueva funcionalidad con características específicas\", \"status\": \"pendiente\", \"requestedAt\": \"$REQUESTED_AT\"}"  # Usar requestedAt, createdAt es automático
